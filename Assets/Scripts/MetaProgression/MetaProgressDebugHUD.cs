...
namespace MetaProgression
{
    ...
    public class MetaProgressDebugHUD : MonoBehaviour
    {
        ...
        void OnGUI()
        {
            ...
            GUILayout.Label($"集計: HP+{s.hpBonus} / Gold+{s.goldBonus} / Dice+{s.diceTotalBonus} / DmgRed-{s.damageReduce} / Hunger-{s.hungerReduce} / Mat+{s.startMaterial} / WinG+{s.combatGoldBonus}",
            ...
            GUILayout.Label($"Refund Lv{s.refundLevel} ({MetaBuffApplicator.GetRefundChance() * 100f:0}%) / Crit Lv{s.critLevel} / BossExtra: {bossExtra}",
                new GUIStyle(GUI.skin.label) { fontSize = fontSize, wordWrap = true });
